...
- Shared singletons: `react` and `react-dom` at `^19.2.0`. Signal K Admin provides both with `import: false`, so the panel has no bundled React fallback. Never set `strictVersion` on these shares: the Admin registers them with a hardcoded 19.0.0 while actually shipping a newer React (2.24.0 bundles 19.2.4), so a strict `^19.2.0` check rejects a fully compatible host and, with `import: false`, the panel never mounts there (this shipped in 1.10.8 and forced an immediate patch release). The version-range warning on such hosts is expected and harmless. `signalk-nearlcrews-ui` 0.8.2 and its React Aria dependencies remain bundled in the remote. It is pinned exactly, and `scripts/shared-ui-version.mjs` asserts the pin so a bump that has not been reviewed fails the panel checks: the shared UI ships breaking changes in minor releases. That guard is deliberately two assertions rather than one: an exact-version shape check that the manifest value and the constant must both satisfy, plus the constant itself as the deliberate-bump tripwire. Keeping them separate closes the repair path where the caret range `npm install` writes gets pasted into the constant, which would satisfy an equality-only check while the pin quietly stopped being exact. `scripts/test-shared-ui-version.mjs` covers both and runs in `npm run test:panel`.
