...
Panel UX pieces: `ThemeToggle` (in the top control bar) pins the panel theme to Auto, Light, Dark, or the red-preserving Night mode by writing a `data-skn-theme` attribute on the `.skn-panel` root, persisted under the `skn-theme` localStorage key; the matching token blocks live in `styles.ts` (`THEME_STYLE`). A Configure / Status segmented toggle in the same control bar switches the body between the editor and `StatusView`, a read-only live-emit table (`StatusView status={status} meta={meta}`). `FirstRunWizard` is a guided modal that fetches `GET /api/paths`, matches the observed Signal K paths against each conversion's declared paths, and proposes the conversions with live data (grouped by category, pre-checked) plus the preset shortcuts; Apply stages the enables through the reducer and the user reviews and Saves. A first-run callout above the catalog opens it whenever the status snapshot reports zero enabled conversions. A debounced catalog search above the tabs filters by title, PGN number, and Signal K path across all categories. The footer is sticky at the bottom of the viewport, a `beforeunload` guard fires while dirty, and Save / Discard move focus to the save-status element so keyboard focus is not dropped when those buttons disable themselves. Conflict-safe saves: `useConfig` keeps the saved config as a baseline and, when an external `configuration` prop arrives while the panel is dirty (an advisor write or scheduled review), runs a three-way merge (`mergeExternalConfig`) that adopts external changes for every key the user has NOT touched (per-conversion entry, plus `globalResendInterval` and the advisor block) while keeping the user's edits for keys they HAVE touched, then updates the baseline so the next Save persists the merged result.
