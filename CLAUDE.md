...
Signal K NMEA 2000 Emitter Cannon is a TypeScript Signal K server plugin that converts Signal K marine navigation data to NMEA 2000 format. It ships 52 conversion modules that expand into 83 runtime conversions emitting 61 distinct data PGNs, with Garmin compatibility. `pgnList.ts` announces one plugin-owned PGN, 126464, in the transmit list; provider-owned transport PGNs are deliberately excluded because output readiness does not prove the provider emits any particular one.
...
- `src/utils/validation.ts` - Input validation (`isValidNumber`, `toValidNumber` - rejects NaN/Infinity to null, `toFiniteInRange(value, min, max)` - the same coercion but to undefined, and only inside an inclusive wire range, which is how every narrow NMEA 2000 numeric field is guarded), `toUnsignedAngle()` (the single choke point for unsigned uint16 angle fields: nullish passes through, non-finite returns undefined, everything else is wrapped to [0, 2pi); the encoder otherwise wraps negatives by the field modulus, not 2 pi), `isPlainObject()` (shared unknown-narrowing guard used by migrate, the router, the advisor, and the panel), `clampString()` (truncates a value so it cannot overflow an NMEA 2000 string field), `resolveInstanceAndSource(options, defaultInstance, defaultSource, allowedSources)` (the shared reader for the per-conversion `instance` and `n2kSource` extras on the temperature and humidity factories: reads the flattened production option shape, falls back to the per-source default, and clamps the instance to the encodable uint8 data range)
...
- `src/utils/aisUtils.ts` - AIS shared helpers: `starboardOffset()` (NaN-safe beam/offset to from-starboard conversion), `parseMmsi()`, `parseImo()`, `AisShipType` interface, string-length caps (`AIS_NAME_CHARS`, `AIS_CALLSIGN_CHARS`, `AIS_DESTINATION_CHARS`, `AIS_SAFETY_TEXT_CHARS`, `ATON_NAME_CHARS`), and the numeric field widths both AIS modules range check against (`MAX_AIS_SOG_METERS_PER_SECOND`, `MAX_AIS_DECIMETER_FIELD`, `MAX_AIS_ANGLE_RADIANS`); imported by `ais.ts`, `aisExtended.ts`, and `dscCalls.ts`
...
- `src/constants.ts` - Standard N2K values (`N2K_DEFAULT_PRIORITY`, `N2K_BROADCAST_DST`, `N2K_DEFAULT_SID`, `N2K_SID_ZERO`, `N2K_DEFAULT_INSTANCE`, `MAX_TANK_INSTANCE`, `MAX_N2K_INSTANCE` (252, the highest encodable value of the 8-bit temperature/humidity instance field), the wire ceilings `MAX_N2K_SPEED_MPS`, `MAX_TEMPERATURE_K`, `MAX_TEMPERATURE_EXTENDED_K`, `MAX_PRESSURE_PA`, and `MAX_N2K_DOP`, `DEFAULT_DATA_TIMEOUT_MS`, `VESSELS_SELF_CONTEXT`, `STREAM_DEBOUNCE_MS`)
...
- `src/conversions/routeTypes.ts` - the shared `Position` interface and `markTypeFor()`, the canboat MARK_TYPE mapping used by PGN 129301 and 129302. The route-list PGNs 129285 and 130074, and the fast-packet byte-budget packing they needed, were removed; do not reintroduce their helpers here without the conversions that use them.
...
- `src/conversions/windData.ts` - `createWind130306Conversion()`: the shared PGN 130306 (Wind Data) conversion factory used by the apparent, true-over-water, true-over-ground, and weather-forecast wind modules, plus `buildWind130306Message()`, the field layout on its own for `windWeatherTrue.ts`, which derives its angle from two inputs and so cannot use the factory
...
1. Loads every conversion module (the registry expands the 52 source modules into 83 runtime conversion objects via the per-instance factories; `index.test.ts` pins that count)
...
`alertTextDescription` (PGN 126985, a STRING_LAU field) is clamped to `MAX_ALERT_TEXT_BYTES = 200`. The binding constraint is the fast-packet limit: 32 frames carry 6 + 31 * 7 = 223 bytes, and at exactly 200 characters this PGN is already 221 payload bytes, so adding any currently omitted field would push it over and the cap has to come down with it. Past the limit canboatjs throws, and signalk-server's `safeApply` re-raises that throw on a bare `setTimeout`, so no plugin try/catch can intercept it and the host process dies. The overflow has to be prevented before the emit; clamping the field is the cheapest way (a general pre-emit PGN size guard would be the broader alternative).

`alertId` is a 16-bit unsigned NMEA 2000 field; the allocator caps at `MAX_ALERT_ID = 65532` (the field's `RangeMax`, so the canboat encoder never sees the "data not available" sentinel) but in practice the active set is bounded by `MAX_TRACKED_PATHS = 256` (one entry in `ids` per active path, released on a clear state or LRU eviction). The eviction really is least-recently-seen: `bindAlertId` and `setAlertPgns` delete before set so a refreshed alert moves to the newest end. Without that a long-running anchor alarm was the first thing evicted at the cap, and it flapped on the chartplotter.
