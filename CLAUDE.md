...
Tests live in `src/test/` across 14 files (`advisor-config.test.ts`, `advisor.test.ts`, `api.test.ts`, `discovery.test.ts`, `index.test.ts`, `lifecycle.test.ts`, `migrate.test.ts`, `pathUtils.test.ts`, `rowStatus.test.ts`, `schedule.test.ts`, `smoothing.test.ts`, `status.test.ts`, `temperature.test.ts`, `useConfig.test.ts`). The conversion-module test cases live embedded in each module's `tests` array, run by `src/test/index.test.ts`. The full suite (140 tests):
...
- **Build**: esbuild bundles to single `dist/index.mjs` (currently ~363 KB)
...
The `/conversions` catalog must be served even when the plugin is disabled. signalk-server calls `registerWithRouter` for every loaded plugin but only calls `start()` (which builds `PluginManager`) once the plugin is enabled, so a disabled plugin has no manager and the panel would otherwise show every category at zero with nothing to configure (the one moment the catalog is needed). The catalog is pure module metadata, so the mapping lives in `buildConversionMetadata()` (`src/api/conversion-metadata.ts`) and `PluginManager.getConversionMetadata()` delegates to it. `index.ts` builds one shared `getMetadata` provider, injected into both the API router (`/conversions`) and the advisor: it returns the running manager's catalog when started, else a standalone copy built once from `createConversionModules(app, plugin)` and cached (lazily, so an enabled plugin never builds the throwaway copy).
...
Layout: conversions render as dense rows (`src/panel/components/ConversionRow.tsx`, about 34px tall) inside one hairline-divided bordered container per section (`S.rowList`). Each row carries, left to right, a 3px left status rail, an enable checkbox, a disclosure toggle (title plus the PGN run), a compatibility/legacy badge dot with a visually-hidden text label, an error glyph, and a right-aligned recency column. The pure helper `rowStatus(status, enabled)` in `src/panel/rowStatus.ts` derives `{ rail, recency }`: emitting is a solid rail (`--skn-ok`), enabled-but-silent is a dotted rail (`--skn-wait`), error is `--skn-danger-fg` plus the glyph, and disabled has no rail. This is night-theme safe: night `--skn-ok` and `--skn-wait` are both amber, so emitting versus silent is carried by the always-present recency text and the solid-versus-dotted rail pattern, not by hue. Editing is a single-open inline accordion: clicking a row expands `ConversionDetail.tsx` (the lifted editor body: resend interval, one source field per Signal K path, the extras editors, and the purpose/note/compatibility prose) full width below the row, and opening another row closes the previous one. The panel holds a single `expandedKey: string | null` with a referentially stable toggle, and focus returns to the row's toggle on collapse.

`PanelToolbar.tsx` (a labelled `<section>` landmark, `position: sticky` via a `--skn-toolbar-height` variable) carries the catalog search, a condensed status chip (a dot, the enabled-over-total count, a readiness word, a visible stale-poll marker, and the `ErrorBadgeButton` jump-to-error), the Configure / Status `SegmentedControl`, the `ThemeToggle`, and the Setup wizard shortcut. Quick presets (`PresetChips`), the Config Advisor (`AdvisorPanel`), and Global settings collapse into three one-line `Disclosure`s below the toolbar (state keys `panel:presets`, `panel:advisor`, `panel:global`); the Config Advisor disclosure is non-lazy so its review and pending state survive collapse. Each category section header (`CollapsibleSection`) gained Enable all and Disable all controls via a `Disclosure` `headerTrailing` slot (so the bulk buttons are valid siblings outside the toggle button); they act on that section's keys via `setEnabledForKeys(keys, enabled)`, announce through `role="status"`, and appear only in the tab view, never on a search-result section.

`ThemeToggle` pins the panel theme to Auto, Light, Dark, or the red-preserving Night mode by writing a `data-skn-theme` attribute on the `.skn-panel` root, persisted under the `skn-theme` localStorage key; the matching token blocks live in `styles.ts` (`THEME_STYLE`). The Configure / Status toggle switches the body between the editor and `StatusView`, a read-only live-emit table (it takes the parent's memoized `metaByKey` and resolves sub-conversion rows via `stripSubIndex`); both views stay mounted (swapped with the `hidden` attribute) so advisor state survives the switch. `FirstRunWizard` is a guided modal that fetches `GET /api/paths`, runs the advisor's pure `recommend()` over the observed paths, and proposes the enable-action conversions (grouped by category, pre-checked) plus the preset shortcuts, so its proposals always agree with Review now; Apply stages the enables through the reducer and the user reviews and Saves. A first-run callout above the catalog opens it whenever the status snapshot reports zero enabled conversions. The footer is sticky at the bottom of the viewport, a `beforeunload` guard fires while dirty, and Save / Discard move focus to the save-status element so keyboard focus is not dropped when those buttons disable themselves. New `styles.ts` blocks cover the dense row, the rail, the hairline container, the toolbar, and the badge-dot (the dot uses the foreground tokens `--skn-warn-fg` and `--skn-text-muted`), plus injected `.skn-row` hover and `scroll-margin-top` CSS and the `--skn-toolbar-height` variable; no new color tokens were added. Conflict-safe saves: `useConfig` keeps the saved config as a baseline and, when an external `configuration` prop arrives while the panel is dirty (an advisor write or scheduled review), runs a three-way merge (`mergeExternalConfig`) that adopts external changes for every key the user has NOT touched (per-conversion entry, plus `globalResendInterval` and the advisor block) while keeping the user's edits for keys they HAVE touched, then updates the baseline so the next Save persists the merged result.
