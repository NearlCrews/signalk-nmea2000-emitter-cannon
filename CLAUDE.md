...
- **Root**: `README.md`, `CHANGELOG.md`, `LICENSE`, `THIRD_PARTY_NOTICES.md`, `CLAUDE.md`, and tooling files only (`package.json`, `biome.json`, `tsconfig*.json`, `vitest.config.mts`, `webpack.config.cjs`). Do not add other docs to the root. `THIRD_PARTY_NOTICES.md` is a license artifact rather than documentation: it ships in the tarball beside `LICENSE`, is generated by `npm run licenses`, and is verified by `npm run package:check`. Never hand-edit it.
...
- **`docs/decisions/`**: design-decision and spike memos (dated, e.g. the React config panel design, the Config Advisor design, and the panel density redesign design). These are historical records of why something was built the way it was; they are not kept current, and the implementation plans that once sat beside them are gone because their work shipped.
...
Signal K NMEA 2000 Emitter Cannon is a TypeScript Signal K server plugin that converts Signal K marine navigation data to NMEA 2000 format. It ships 52 conversion modules that expand into 83 runtime conversions emitting 60 distinct data PGNs, with Garmin compatibility. `pgnList.ts` announces one plugin-owned PGN, 126464, in the transmit list, for 61 in total; provider-owned transport PGNs are deliberately excluded because output readiness does not prove the provider emits any particular one.
...
All 52 modules in `src/conversions/` follow this factory pattern:
...
- `src/utils/refreshInterval.ts` - `resolveRefreshInterval(conversion)`: the stream-refresh interval in milliseconds for an on-value-change conversion, or undefined when the module declares none
- `src/utils/pgnPriorities.ts` - `CANBOAT_PGN_PRIORITIES`, the Canboat 7.1.0 priority per emitted PGN, applied at the emit boundary; proprietary and undefined-priority PGNs keep the conversion's own value
- `src/utils/debugUtils.ts` - `isDebugEnabled(app)`: reads the debug instance's own gate so an expensive payload is never built for output nobody collects
...
- **Build**: esbuild bundles to single `dist/index.mjs`. Its budget is the `size-limit` entry in `package.json`: 70 kB brotli, checked by `npm run size`. The enforced figure is the only one recorded here, because a figure measured by hand goes stale the next time a conversion lands and nothing catches it.
...
- **Wire bounds**: `MAX_N2K_ANGLE_SIGNED_RADIANS` is 3.1415, deliberately below `Math.PI` and below canboat's own stated `RangeMax` of 3.1415926. canboat's decoder discards any field whose raw value exceeds `RangeMax / Resolution`, and 3.1415926 / 0.0001 is 31415.926, so raw 31416 is thrown away at the far end while raw 31415 survives. A guard written against `Math.PI` admits everything in (3.1415, pi], which encodes onto the bus and then vanishes. Do NOT "correct" this constant upward. `MAX_N2K_ANGLE_UNSIGNED_RADIANS` is 6.2831 for the same reason, below a full turn rather than at it. The same trap sits in `MAX_N2K_VOLTAGE_V`: canboat states 327.64, but 327.64 / 0.01 evaluates to 32763.999999999996 in IEEE 754, so 327.63 is the largest value that round-trips. When a bound looks a hair low, read its comment before touching it.
- **Wrap versus drop**: `toSignedAngle` and `toUnsignedAngle` WRAP, because an angle is circular and 4 rad names the same direction as -2.2832 rad, then clamp the residue onto the encodable ceiling. Every other bound DROPS, because a rudder deflection, a rate of turn, a heave, a depth, a voltage, or an engine speed is a bounded magnitude, so a value past the ceiling is bad data rather than the same quantity stated differently. Pick the right one for a new field: wrapping a magnitude invents a reading, and dropping an angle discards a good one.
- **Detached bus writer**: `app.emit("nmea2000JsonOut", ...)` returns false when no listener is attached, which is what a disabled or torn-down NMEA 2000 connection looks like from here. `PluginManager.noteBusWrite` trips after `UNDELIVERED_WRITE_RUN` (20) consecutive undelivered writes and calls `setPluginError`; the first delivered write clears it. The run length exists because a provider restart briefly drops the listener. This check is necessary because signalk-server sets `isNmea2000OutAvailable` once and never resets it, and the plugin latches its own readiness the same way, so without it a disabled connection leaves the status reading "Running" while every frame goes nowhere. Only an explicit `false` counts: an older server or a host returning nothing must not be read as detached. `/api/status` exposes the state as `busWriterAttached`.
...
Layout: conversions render as dense rows (`src/panel/components/ConversionRow.tsx`) inside one hairline-divided bordered container per section. Their complete style contract lives in `conversionStyles.ts`, the only panel style module left; every color, spacing, radius, and type value in it reads a public `--snui-*` token, and the panel injects no stylesheet of its own. Each row carries, left to right, a 3px left status rail, a shared `Checkbox` with a hidden label (`Enable <title>`), a disclosure toggle (title plus the PGN run), a neutral `Badge` reading Legacy on superseded conversions, a compact danger `StatusIndicator` whose hidden text is the error message, and a right-aligned muted recency column. The toggle is a text-styled native button wired through the library's `useDisclosure` hook (ids, `aria-expanded`, `aria-controls`, the hidden region, and focus handoff into the editor on open and back to the trigger on close); it is deliberately not the library `Button`, whose centered action geometry does not fit a list line, and it relies on the browser's own focus ring. Rows sit at the shared control floor (`--snui-control-min-height`: 40px, 44px under a coarse pointer), which the library checkbox and buttons carry themselves, so no panel media query is needed for touch targets. The pure helper `rowStatus(status, enabled)` in `src/panel/rowStatus.ts` derives `{ rail, recency }`: emitting is a solid rail (`--snui-color-success`), enabled-but-silent is a dashed rail (`--snui-color-warning`), error is `--snui-color-danger` plus the indicator, and disabled has no rail. The rail lives on the row header so it stays a short tick and does not run the full height of the open editor. This is night-theme safe: night success and warning are both in the red family, so emitting versus silent is carried by the always-present recency text and the solid-versus-dashed rail pattern, not by hue. Editing is a single-open inline accordion: clicking a row expands `ConversionDetail.tsx` (the lifted editor body: a `NumberField` for the resend interval, one `SourceField` per Signal K path inside a `FieldGroup`, the extras editors, and the purpose, note, and compatibility prose) full width below the row in a recessed region, and opening another row closes the previous one. The panel holds a single `expandedKey: string | null` with a referentially stable toggle. The row wrapper keeps the panel-owned id `skn-row-<key>`; the toggle and region ids are generated by the hook, so jumps locate the toggle as the row's `[aria-expanded]` element.

`PanelToolbar.tsx` (a labelled `<section>` landmark, sticky to the top of the panel by inline style) carries the catalog search (`TextInput`), a live `StatusIndicator` chip (the enabled-over-total count, a readiness word, and a stale-poll marker, toned success when ready and warning while waiting), the `ErrorBadgeButton` jump-to-error (its count arrives as a prop: the panel tallies the parent rows reporting an error in the same pass that builds the error-key set, so a poll walks the status list once), the Configure / Status `SegmentedControl`, and the Setup wizard `Button`. The configure view is a `Stack` of shared sections: Quick presets (`CollapsibleSection`, `mountStrategy="unmount"`, holding `PresetChips`), the Config Advisor (`AdvisorPanel`, its own `CollapsibleSection` with an Advisor settings section at heading level 3 and `Section` headings at level 4 for the QuestDB and schedule groups), Global settings (`GlobalSettings`, a `CollapsibleSection` whose header summary shows the effective interval), and a Conversions `Section` that holds the library `Tabs` (one `Tab` per category with a danger `Badge` count when a category reports errors, and `TabPanel`s that unmount while unselected) or, while searching, one section per matching category. Each category section (`CatalogSection`, built on `CollapsibleSection` at heading level 3 with `mountStrategy="unmount"`, a header summary of counts, and Enable all and Disable all `Button`s in the `actions` slot announcing through a `LiveRegion`) acts on that section's keys via `setEnabledForKeys(keys, enabled)`; the bulk buttons appear only in the tab view, never on a search-result section. That gives the configure view a heading outline from h2 (Quick presets, Config Advisor, Global settings, Conversions) through h3 (Advisor settings, Modern, Legacy) to h4 (the advisor groups, Mapping activity, Advanced publisher filters, and a row's remove confirmation).

`ThemeToggle` selects Match Admin, Match device, Light, Dark, or the red-preserving Night mode. Explicit themes write `data-snui-theme` on the panel root and persist through the shared `signalk-nearlcrews-ui.theme.v1` localStorage key. Match Admin is the fresh-profile default, does not write a root theme attribute, follows an explicit Signal K host theme, and otherwise uses Light. Match device follows the operating system preference. The two automatic choices carry the stored values `auto` and `system`. `scripts/check-panel-browser.mjs` asserts the part this panel owns: a fresh profile selects Match Admin and writes no `data-snui-theme` on the root, and a chosen theme reaches the root (it clicks Night and expects `night`). Which theme each of the five choices commits is the library's own contract, covered by the library's suite rather than duplicated here. The retired `skn-theme` key is intentionally ignored. The panel frame is `PanelShell` (`themeToggle="end"`, written out rather than left to the default: the placement is a house convention across the plugin panels built on these controls, so it is stated here rather than inherited from a library default that could move): it renders the selector at the foot of the panel, runs the CSS `@scope` preflight and renders the library's browser notice when it fails, paints the root, and wraps the body in `PanelErrorBoundary`, whose Try again remounts the panel in place and whose Reload page action (`onReload`) reloads the Admin page. The shell also mounts the panel's one polite and one assertive live region before any message exists, so panel-level announcements (a catalog load failure, a blocking validation error, a status outage) are spoken through `usePanelAnnouncer()` and the panel mounts no region of its own for them: a region created together with its first message is not announced reliably. The Configure / Status toggle switches the body between the editor and `StatusView`, a `Section` with a `MetricGrid` readout and a zebra `Table` inside a `TableScrollRegion`; both views stay mounted (swapped with the `hidden` attribute) so advisor state survives the switch. `FirstRunWizard` is a shared `Dialog` (the library owns the scrim, focus trap, focus return, Escape, and viewport sizing) that fetches `GET /api/paths`, runs the runtime-neutral `recommend()` from `src/recommendation/` over the observed paths, and proposes the enable-action conversions as one `CheckboxGroup` per category, pre-checked, plus the preset shortcuts, so its proposals always agree with Review now; Apply stages the enables through the reducer, and the user reviews and saves. A first-run callout above the catalog opens it once the catalog is loaded and the current panel configuration has no enabled catalog conversion. The footer is the library `SaveActionBar` (docked to the viewport bottom, Save refused through `aria-disabled` per the shared rule and while `invalidMessage` names blocking validation errors, so Save and Discard keep their place in the tab order while validation blocks them, focus moved to its status after Save or Discard) paired with `useUnsavedChangesGuard(dirty)`; the panel keeps only the save state itself and the 2.5 second `saveRequestedAt` window. Mapping tables (`extras/MappingTable.tsx`) are the library `Table` inside a `TableScrollRegion` named by the caption; text and select cells are `TextInput` and `Select` named by their column header, number cells are a bare `NumberInput` driven by `useNumberDraft` (a labeled `NumberField` would repeat the header in every cell), and Remove opens an `InlineConfirm` in a row beneath so the Remove button never changes its own name. Data rows carry `data-mapping-row` so the validation jump indexes past an open confirmation. Conflict-safe saves: `useConfig` keeps the last requested configuration as a baseline and, when an external `configuration` prop arrives while the panel is dirty (an Advisor write or scheduled review), runs a three-way merge (`mergeExternalConfig`) that adopts external changes for every key the user has not touched while keeping the user's edits on conflict, then updates the baseline so the next Save sends the merged result. Semantic no-op actions preserve identity and do not mark the panel dirty.
...
- Shared singletons: `react` and `react-dom`, spread from `require("signalk-nearlcrews-ui/federation")` in `webpack.config.cjs`. Signal K Admin provides both with `import: false`, so the panel has no bundled React fallback. Never set `strictVersion` on these shares: Admin releases up to at least 2.24.0 register their React share as 19.0.0 while shipping a newer React (current master registers the real version), so a strict `^19.2.0` check rejects a fully compatible host and, with `import: false`, the panel never mounts there (this shipped in 1.10.8 and forced an immediate patch release); the entry's `hostNotes` string records the same reason. The version-range warning on such hosts is expected and harmless. `signalk-nearlcrews-ui` and its React Aria dependencies remain bundled in the remote. It is pinned exactly (currently 0.11.1), because the shared UI ships breaking changes in minor releases, and `npm run test:panel` runs the library's own `snui-check-consumer` after the panel build: it asserts that the exact pin equals the installed version, that every chunk carries that version's `data-snui-version` stamp and no other, that no React runtime was bundled, that the remote consumes and the config declares the published share map, and that the remote's gzip size stays within `scripts/panel-size-baseline.json` (`gzipBytes` plus `maximumIncreasePercent`; raise the baseline deliberately after reviewing what grew). Its `--runtime` mode then renders the built remote through a host-equivalent share scope and asserts the production JSX runtime, the compatibility notice, and the shared root. The notice assertion is the library's own structural marker: this repository used to run a local harness that matched the literal string "Browser update required", which is library user-facing copy hand-copied here, so a rewording in the library would have silently stopped testing anything. Do not reintroduce a local copy of library copy as a test anchor. The render anchor is `--expect "Setup wizard"`, chosen because `scripts/check-panel-browser.mjs` already asserts that same button name, so the two gates ride one contract and renaming the button fails both, which is the intended coupling. Dependabot groups `signalk-nearlcrews-ui` alone so a 0.x minor arrives as its own pull request with the migration notes open.
...
4. Bound every numeric field against the wire before emitting it. A canboatjs field does NOT reject an out-of-range value: it wraps by the field modulus, so bad input arrives as a different, believable reading rather than as not-available, which is worse than sending nothing. Use `toFiniteInRange` against a named ceiling in `src/constants.ts` (add one there rather than inlining a literal), and bound AFTER any unit conversion, in the units the wire carries. Choose the failure mode deliberately: a circular quantity wraps through `toSignedAngle` or `toUnsignedAngle` because 4 rad and -2.2832 rad name the same direction, while a bounded mechanical quantity such as a rudder deflection is dropped because an out-of-range value is simply bad data. Drop the whole frame when nothing encodable is left.
5. Add test cases in the module's `tests` array. A range regression case is not done until it has been shown to FAIL against the pre-fix bound: temporarily restore the old bound, watch the case fail, then put the fix back. Two ways to get a case that is green for the wrong reason, both hit during the 1.11.0 work. Picking an input the old bound ALREADY rejected, such as 3.1416 against a `Math.PI` guard, passes either way and discriminates nothing; the input has to sit between the old bound and the new one, which for that guard means `Math.PI` itself. And an input chosen by eye near a ceiling can land on the wrong side of it in IEEE 754: 6.28315 looks like it sits inside the band canboat's decoder discards but does not, so measure the value rather than estimating it.
...
Optional subsystem (added v1.6.0) that reviews observed Signal K paths and recommends which conversions to enable or disable, and flags enabled conversions whose per-path `$source` pin no longer matches a live source. It is dormant unless `advisor.enabled` is set and adds no work to the emit hot path. It is fully deterministic and makes no outbound LLM calls (the earlier optional OpenRouter rationale enrichment was removed). The design spec is `docs/decisions/2026-05-16-openrouter-config-advisor-design.md`, a dated record of the original four-phase shape rather than a description of what shipped.
...
- `canboatjs-nmea2000`: PGN definitions, encoding, custom PGNs, and Actisense output.
...
- `openrouter-llm`: natural-language configuration generation through OpenRouter and its validation loop.
