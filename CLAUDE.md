...
- Shared singletons: `react` and `react-dom` at `>=19.2.0 <20.0.0`. Signal K Admin provides both with `import: false`, so the panel has no bundled React fallback. `signalk-nearlcrews-ui` and its React Aria dependencies remain bundled in the remote.
