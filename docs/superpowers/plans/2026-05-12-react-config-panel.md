...
**Goal:** Replace the hand-rolled JSON Schema admin UI for `signalk-nmea2000-emitter-cannon` with a federated React panel (webpack 5 Module Federation), TypeBox-derived config schema, and live discovery/status endpoints over an Express router. Target release: v1.5.2.
...
- Screenshots are optional for v1.5.2 release. If skipping, omit the screenshots section entirely from the README. Do NOT write placeholder text like "Screenshots coming soon"; either include a real screenshot or omit the section.

- [ ] **Step 2: Commit**

```bash
git add README.md
git commit -m "docs(readme): describe React config panel and v1.5.2 admin UI requirement"
```
...
git commit -m "docs(changelog): v1.5.2 entry"
```
...
```bash
npm version --no-git-tag-version 1.5.2
```
...
```bash
git add package.json package-lock.json
git commit -m "release: v1.5.2 React config panel"
git tag -a v1.5.2 -m "v1.5.2 React config panel (local only, not pushed)"
```
...
Final state to report: "v1.5.2 staged locally as tag v1.5.2. No push performed. Waiting for ok to push."
