...
The migrated object is written back only on the next user-driven save through the panel. A user who downgrades to v1.4.4 after seeing v2.0.0 still has their original `plugin-config.json` intact.
...
7. Preset chips.
8. Documentation rewrite (README, CHANGELOG), v2.0.0 release candidate. Local commit only, no push.
