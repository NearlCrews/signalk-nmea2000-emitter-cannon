...
export default function createAisExtendedConversions(): ConversionModule[] {
  return [
    // AIS Class B Position Report (PGN 129039)
    {
      ...
      callback: (
        ...
        // Calculate position reference points (not used in this PGN)
        // let _fromStarboard: number | null = null;
    ...
    // AIS Class B Extended Position Report (PGN 129040)
    {
      ...
      callback: (
        ...
        const shipTypeName = (shipType as AisShipType)?.name || "Sailing";
    ...
    // AIS SAR Aircraft Position Report (PGN 129798)
    {
      ...
      callback: (aisClass: unknown, position: unknown, cog: unknown, sog: unknown, altitude: unknown): N2KMessage[] => {
        if (aisClass !== "SAR" || 
            typeof position !== 'object' || 
            position == null ||
            !(position as Position).latitude || 
    ...
    // AIS Safety Related Broadcast Message (PGN 129802)
    {
      ...
      callback: (safetyMessage: unknown, seqId: unknown): N2KMessage[] => {
