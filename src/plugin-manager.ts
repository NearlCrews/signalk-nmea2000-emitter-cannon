...
export class PluginManager {
    ...
    try {
      ...
      for (const conversion of this.conversions) {
        ...
        for (const conv of conversionArray) {
          ...
          this.app.debug(`Checking conversion ${conv.title} (${conv.optionKey}) - enabled: ${convOptions?.enabled}`)
          ...
          for (const subConversion of subConversions) {
            ...
            this.app.debug(`Mapper completed for ${subConversion.title || 'unnamed subconversion'}`)
          }
        }
      }
      ...
      this.app.error(`Failed to start plugin: ${error instanceof Error ? error.message : String(error)}`)
  ...
  private mapRxJS(conversion: ConversionModule, options: unknown): void {
    ...
    const subscription = combinedBus.pipe(
      ...
      try {
        this.app.debug(`*** CALLBACK TRIGGERED for ${conversion.title} with values: ${JSON.stringify(values)}`)
        if (conversion.callback) {
